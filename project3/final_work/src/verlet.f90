...
!---------------------------------------------------------------------------------------!
!-Module to create explicit interface of allocatable dummy arguments:-------------------!
!-it contains the subroutine that implements the Verlet algorithm-----------------------!
                                                                                        !                  
  module verlet                                                                         !         
  use distances                                                                         !
  use accel                                                                             !
  contains                                                                              !           
   subroutine implement_verlet(coord,natoms,vel,dt,acceleration,col,d,eps,sigma,mass,f) !           
   implicit none                                                                        !          
   integer, intent(in)             :: natoms, col                                       !
   double precision, intent(inout) :: coord(natoms,3)                                   !
   double precision, intent(in)    :: mass(natoms)                                      !
   double precision, intent(inout) :: d(natoms,natoms)                                  !
   double precision, intent(inout) :: vel(natoms,3)                                     !  
   double precision, intent(inout) :: acceleration(natoms,3), f(natoms,natoms)          !
   double precision, intent(in)    :: eps, sigma, dt                                    !         
   integer                         :: i, j, k                                           !         
                                                                                        !        
   do i=1,natoms                                                                        !              
    do k=1,3                                                                            !       
                                                                                        !                        
    coord(i,k)=coord(i,k)+vel(i,k)*dt+0.5d0*acceleration(i,k)*dt**2                     !          
    vel(i,k)=vel(i,k)+0.5d0*acceleration(i,k)*dt                                        !
                                                                                        !
    enddo                                                                               !                             
   enddo                                                                                !                             
                                                                                        !                        
   ...
   enddo                                                                                !                              
                                                                                        !                        
  end subroutine implement_verlet                                                       !                                                       
                                                                                        !                        
  end module verlet                                                                     !                                         
                                                                                        !                        
!---------------------------------------------------------------------------------------!                         
...
!-declaration block---------------------------------------------------------------------!         !                       
  ...
  double precision :: eps, sigma, V, LJtot, Ttot, T, E, Etot, dt                        !         !
...
!---------------------------------------------------------------------------------------!         !   
                                                                                                  ...
                                                                                                  !   
!---------------------------------------------------------------------------------------!         !     
!-computing LJ potential energy after reading input potential parameters----------------!         !      
                                                                                        !         ! 
...
!---------------------------------------------------------------------------------------!         !                   
                                                                                                  ...
                                                                                                  !                      
!---------------------------------------------------------------------------------------!         !                      
!-Verlet algorithm: it allow us to update coordinates for each time step dt-------------!         !                         
                                                                                        !         !                        
                                                                                        !         !                           
   open(30,file='trajectory.xyz')                                                       !         !                             
                                                                                        !         !                              
   write(30,*) natoms                                                                   !         !                            
   write(30,*) "total energy = LJtot + Ttot =", LJtot, "+", Ttot, "=", Etot             !         !                              
                                                                                        !         !                              
                                                                                        !         !                             
   do i=1,natoms                                                                        !         !                                  
     write(30,'(A2,3F15.6)') "Ar ", (coord(i,j),j=1,col)                                !         !                             
   enddo                                                                                !         !                         
                                                                                        !         !                       
     dt=0.02d0                                                                          !         !                                   
   do n=1,1000                                                                          !         !                                 
                                                                                        !         !                         
     call implement_verlet(coord,natoms,vel,dt,acceleration,col,d,eps,sigma,mass,f)     !         !    
                                                                                        !         !  
     if(mod(n,10).eq.0) then                                                            !         !                                   
       ...
       Etot=E(LJtot,Ttot)                                                               !         !                                   
                                                                                        !         !                              
       write(30,*) natoms                                                               !         !                          
       write(30,*) n, "total energy  = LJtot + Ttot =", LJtot, "+", Ttot, "=", Etot     !         !                               
                                                                                        !         !                                  
       do i=1,natoms                                                                    !         !                                  
         write(30,'(A2,3F15.6)') "Ar ", (coord(i,j),j=1,col)                            !         !                                    
       enddo                                                                            !         !                              
